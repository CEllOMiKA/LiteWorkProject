...
namespace ScreenShotLite
{
    public partial class Main : Form
    {
        ...
        /// <summary>
        /// CaptureAndSave 增强说明：
        /// - 支持通过 PrintWindow/BitBlt/Alt+PrintScreen 捕获所选窗口（若选择了 comboBox1 的窗口），否则使用当前活动窗口。
        /// - 屏幕截图支持选择特定显示器（comboBox2），若未选择则使用虚拟屏幕。
        /// - 当 forceSingle 为 true 时：忽略自动模式，始终执行单次截图（用于热键）。
        /// - 捕获成功后将把使用的窗口/屏幕设置回相应 ComboBox，保证 UI 显示为“上一次截图”。
        /// </summary>
        private string? CaptureAndSave(bool forceSingle = false)
        {
            ...
            try
            {
                if (radioButton2.Checked) // 全屏或选择屏幕
                {
                    ...
                    // record used screen bounds for UI update
                ...
                else if (radioButton1.Checked) // 窗口（可选 comboBox1 中选择的窗口）
                {
                    ...
                    // record used window for UI update
                    usedWindow = hWnd;

                    // 尝试多种窗口捕获方法，按可靠性和兼容性顺序
                ...
                // 更新 UI：确保两个 ComboBox 显示“上一次截图”的选项
                try
                {
                ...
                {
                    Log($"更新 ComboBox 选择失败：{ex.Message}");
                }
        ...
        // 将指定窗口在 comboBox1 中选中（若不存在则尝试加入）
        private void SelectWindowInCombo(IntPtr hWnd)
        {
            ...
            {
                title = $"窗口 0x{hWnd.ToInt64():X}";
            }

            var item = new WindowItem(hWnd, title);
            comboBox1.Items.Insert(0, item);
            comboBox1.SelectedIndex = 0;
        }

        // 根据屏幕 bounds 在 comboBox2 中选中对应显示器
        ...
        // 尝试多种方法捕获窗口，返回一个有效 bitmap 或 null
        private Bitmap? TryCaptureWindowRobust(IntPtr hWnd, Rectangle rect)
        {
            Bitmap? bmp = null;

            // 1) 尝试 PrintWindow 带 PW_RENDERFULLCONTENT（2）
            bmp = TryPrintWindow(hWnd, rect, 2);
            if (bmp != null && !IsMostlyBlank(bmp)) return bmp;

            bmp?.Dispose();

            // 2) 尝试 PrintWindow 无标志
            bmp = TryPrintWindow(hWnd, rect, 0);
            if (bmp != null && !IsMostlyBlank(bmp)) return bmp;

            bmp?.Dispose();

            // 3) 尝试 BitBlt 从窗口 DC（适用于很多场景）
            try
            {
                var bt = TryBitBltWindow(hWnd, rect);
                if (bt != null && !IsMostlyBlank(bt)) return bt;
                bt?.Dispose();
            }
            catch (Exception ex)
            {
            ...
            // 4) 如果以上均失败，尝试模拟 Alt+PrintScreen（将窗口置前，系统会把活动窗口图像复制到剪贴板）
            try
            {
                var altBmp = TryAltPrintScreenCapture(hWnd, rect);
                if (altBmp != null && !IsMostlyBlank(altBmp)) return altBmp;
                altBmp?.Dispose();
            }
            catch (Exception ex)
            {
                Log($"Alt+PrtSc 捕获异常：{ex.Message}");
            }

            return null;
        }

        // 使用 PrintWindow 到指定大小的 bitmap；flags 通常为 2 (PW_RENDERFULLCONTENT) 或 0
        private Bitmap? TryPrintWindow(IntPtr hWnd, Rectangle rect, int flags)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!ok)
                    {
                        // PrintWindow 返回 false，表示无法直接打印
                        return bmp; // 返回 bmp（后续会检测是否白）
                    }
                    return bmp;
                }
                catch
        ...
        // 使用 BitBlt 从窗口 DC 捕获（通常能捕获更多窗口类型）
        private Bitmap? TryBitBltWindow(IntPtr hWnd, Rectangle rect)
        {
            ...
            try
            {
                ...
                if (hWndDC == IntPtr.Zero) return null;
                ...
                // 从窗口 DC 复制
                bool success = BitBlt(hMemDC, 0, 0, rect.Width, rect.Height, hWndDC, rect.Left - rect.Left, rect.Top - rect.Top, SRCCOPY);
                // 恢复
                ...
                // 注意：Image.FromHbitmap 会创建 GDI 对象的副本，之后可以释放 hBitmap
                return bmp;
            }
            finally
            {
                ...
                if (hWndDC != IntPtr.Zero) ReleaseDC(hWnd, hWndDC);
            }
        }

        // 模拟 Alt+PrintScreen：将目标窗口置前，发送按键，读取剪切板图像，恢复原窗口
        private Bitmap? TryAltPrintScreenCapture(IntPtr hWnd, Rectangle rect)
        {
            ...
            try
            {
                ...
                // 发送 Alt down, PrintScreen, Alt up
                keybd_event(VK_MENU, 0, 0, UIntPtr.Zero); // Alt down
                keybd_event(VK_SNAPSHOT, 0, 0, UIntPtr.Zero); // PrtSc down/up
                ...
                if (Clipboard.ContainsImage())
                {
                    ...
                    try
                    {
                        ...
                        if (bmp.Width >= rect.Width && bmp.Height >= rect.Height)
                        {
                            ...
                            var crop = bmp.Clone(new Rectangle(x, y, rect.Width, rect.Height), bmp.PixelFormat);
                            bmp.Dispose();
                            return crop;
                        }
        ...
        // 检查 bitmap 是否几乎全部白色（用于检测 PrintWindow 导致的“全白”问题）
        private static bool IsMostlyBlank(Bitmap bmp)
        {
            try
            {
                ...
                foreach (var p in points)
                {
                    ...
                    if (c.A == 0) // fully transparent treat as blank
                        whiteCount++;
                    else if (c.R > 240 && c.G > 240 && c.B > 240) // near white
                        whiteCount++;
                }
        ...
        private sealed class SelectionForm : Form
        {
            ...
            private void SelectionForm_MouseDown(object? sender, MouseEventArgs e)
            {
                ...
                _selecting = true;
                // 使用窗体坐标（窗体覆盖虚拟屏幕），后面会 Offset 到屏幕坐标
                _start = e.Location;
                _current = e.Location;
